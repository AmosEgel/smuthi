    ...
    def __init__(self, particle_list, initial_field, layer_system, k_parallel='default', solver_type='LU',
        ...
        if coupling_matrix_lookup_resolution is None:
            ...
            if z_list.count(z_list[0]) == len(z_list):
                ...
                if use_gpu:
                    warnings.warn("volume interpolation on gpu not yet implemented - fall back to cpu")
                if interpolator_kind == 'linear':
                    interpolation_order = 1
                elif interpolator_kind == 'cubic':
                    interpolation_order = 3
                else:
                    warnings.warn(interpolator_kind + " interpolator kind is not implemented, using 'linear' instead.")
                    interpolation_order = 1

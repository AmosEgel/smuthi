...
def test_wr_against_prototype_single_particle_over_substrate():
    wr = smuthi.particle_coupling.layer_mediated_coupling_block(wl, rs1, rs1, laysys_substrate,
                                                                neff_contour, show_integrand=False)
    wr00 = -0.031367838040052 - 0.086576016508040j
    assert (wr[0, 0] - wr00) / wr00 < 1e-4
    wr1910 = 0.110962483421277 + 0.155167528762370j
    assert (wr[19, 10] - wr1910) / wr1910 < 1e-4


if __name__ == '__main__':
    test_wr_against_prototype_single_particle_over_substrate()

...
# outgoing wave
Ex, Ey, Ez = fldex.spherical_vector_wave_function(rx + dx, ry + dy, rz + dz, k, 3, tau, l, m)
...
for tau2 in range(2):
    for l2 in range(1, lmax + 1):
        for m2 in range(-l2, l2 + 1):
            Mx, My, Mz = fldex.spherical_vector_wave_function(rx, ry, rz, k, 1, tau2, l2, m2)
            A = fldex.translation_coefficients_svwf(tau, l, m, tau2, l2, m2, k, [dx, dy, dz])
            Ex2 += Mx * A
            Ey2 += My * A
            Ez2 += Mz * A
...
def test_ab5_versus_prototype():
    ...
    a5, b5 = fldex.ab5_coefficients(3, -1, 2, 2, 3)
    a5matl = 0.235702260395516
    assert abs(a5 - a5matl) / abs(a5) < 1e-7

    a5, b5 = fldex.ab5_coefficients(4, 3, 2, 2, 3)
